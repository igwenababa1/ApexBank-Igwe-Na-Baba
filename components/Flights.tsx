...
import { AirplaneTicketIcon, ArrowLongRightIcon, UsersIcon, CalendarDaysIcon, SpinnerIcon, ShieldCheckIcon, QuestionMarkCircleIcon, InfoIcon } from './Icons';
...
// ... (Sub-components: FlightCard, BookingConfirmationModal, etc.)
    ...
    <div className="bg-slate-200 p-4 rounded-lg shadow-digital-inset space-y-3">
        <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
                <img src={flight.airlineLogo} alt={flight.airline} className="w-6 h-6" />
...
export const Flights: React.FC<FlightsProps> = ({ bookings, onBookFlight, accounts, setActiveView }) => {
    ...
    const handleBook = (sourceAccountId: string) => {
        if (selectedFlight) {
            return onBookFlight({ flight: selectedFlight, passengers: searchParams.passengers, totalPrice: selectedFlight.price * searchParams.passengers }, sourceAccountId);
        }
        return false;
    };
    ...
    return (
        <div className="space-y-8">
             {selectedFlight && <BookingConfirmationModal flight={selectedFlight} passengers={searchParams.passengers} accounts={accounts} onConfirm={handleBook} onClose={() => setSelectedFlight(null)} />}
            <div className="flex justify-between items-start">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">Book Flights</h2>
                    <p className="text-sm text-slate-500 mt-1">Search and book flights using your ApexBank account.</p>
                </div>
            ...
            <div className="space-y-4">
                ...
                {searchResults.length > 0 && (
                    <div className="space-y-4">
                        ...
                        {searchResults.map(flight => <FlightCard key={flight.id} flight={flight} onSelect={() => setSelectedFlight(flight)} />)}
                    </div>
                )}
